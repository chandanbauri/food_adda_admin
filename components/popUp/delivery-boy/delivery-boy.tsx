...
import { askForAcceptingOrder, getListOfDeliveryBoys } from "../../../utilities/functions"
      ...
      if (res) {
        ...
        if (parsedResponse.success) {
          let list: Array<any> = parsedResponse.data.map((boy: any) => {
            ...
            let index = boy.requests.findIndex((item: any) => item.orderId == order.id)
            if (index == -1) {
              return boy
            } else return
          })
      ...
      <div className='w-full my-3 flex items-center justify-center'>
        <button
          ...
          onClick={async () => {
            ...
            if (AssignedDeliveryBoys.length > 0)
              try {
